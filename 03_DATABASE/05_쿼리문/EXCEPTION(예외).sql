...
-- 예외처리의 목표 : 프로그램의 정지가 안되도록 하기 위해서 작성
...
begin
	declare continue handler for 1146 select '해당 테이블이 없어요..' as 'Error_msg';		-- 단일 exception에 대한 처리
   	declare continue handler for 1136 select 'Insert시 value의 column이 다릅니다..' as 'Error_msg';

	select * from usertbl; 
	select * from notable;				-- 예외처리를 했기 때문에 프로그램 정지가 안됨 
	select * from buytbl;
    insert into usertbl values(1);		-- 예외처리를 했기 때문에 프로그램 정지가 안됨 
    select 'Result' as '끝';
end $$
delimiter ;
...
show errors;		-- 에러를 표시, 예외처리한 것은 표시되지 않음
-- 03 모든 예외 받기..
delimiter $$
