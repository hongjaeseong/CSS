...
-- -------------------
-- INDEX 관련 알고리즘 종류 // type or 자료구조 으로 써야하는 지 고민중 // 검색 알고리즘 종류: 순차탐색(), 이진탐색(binary search)
-- --------------------
-- B-Tree	 : 기본값, 대부분의 데이터 index에 잘 적용되어 사용 (이진탐색 기반: 검색을 하면 할 수록 검색범위가 줄어듬)
-- Hash		 : 해시 함수를 이용한 index, 정확한 일치 검색에 사용(포함검색에는 다소 성능이 저하될 수 있다)
-- Full-text : 전체 텍스트 검색에 사용되는 index, 텍스트 검색기능 향상 시 유리
-- Spatial	 : 공간데이터(위도/경도 등을 담는 지도데이터)를 처리하기 위한 Index, 지리 정보 검색에 유리

-- -------------------
-- MYSQL INDEX 종류(TYPE)
-- --------------------
-- 클러스터형(Primary) 인덱스	: PK열에 기본적으로 적용되는 index, 사전편찬 순서에 맞게 정렬이 된다. [기본 : B-Tree]
-- 								: 한테이블에 한개만 생성
-- 								: 실제 데이터의 정렬이 인덱스의 순서로 정렬
-- 								: 보조인덱스보다 빠른 속도

-- 보조(Secondary) 인덱스		: PK이외 다른 제약조건이나 수동으로 설정 시 적용되는 index 검색알고리즘에 맞게 설정된다 [기본 : B-Tree]
-- 								: 한테이블에 여러 Index를 생성
-- 								: 비클러스터형 인덱스라고도 함
...
alter table tbl_b drop primary key;		-- primary key 삭제 -> index 삭제
show index from tbl_b;
desc tbl_b;

alter table tbl_b drop constraint col2;		-- drop constraint -> index 삭제
